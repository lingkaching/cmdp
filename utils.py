...
def cost_t_nt_z(z, nt_z, learnedCost_z, lambda_z):
    # costRes = 0
    ...
    if nt_z == 0:
        ...
        #wDelay is a time step penalty, here it is 1
        costDelay = wDelay
        costRes = lambda_z * max(0, 1-rCap[z]/nt_z)
        # costResHat = lambda_z * max(0, 1-rCap[z]/(nt_z * learnedCost_z+1e-6))
        costResHat = lambda_z * max(0, learnedCost_z-rCap[z]/nt_z)

    # return wDelay+lambda_z
    return float(costDelay+costRes), float(costDelay+costResHat), costRes, costResHat, costDelay
