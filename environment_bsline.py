...
class Maritime:
    ...
    def step(self, t, cT, beta, learnedCost, myLambda):
        ...
        # ---------- Rewards ----------- #
        #rm, rmHat, res, resHat, delay

        self.rt_z, self.rtHat_z, res, resHat, delay = self._atomicReward(self.cT.nt_z, learnedCost, myLambda)
        self.rt = np.sum(self.rt_z * self.cT.nt_z)
        self.rtHat = np.sum(self.rtHat_z * self.cT.nt_z)
        res = np.sum(res * self.cT.nt_z)
        resHat = np.sum(resHat * self.cT.nt_z)
        delay = np.sum(delay * self.cT.nt_z)

        # Normalize the reward
        # self.rt_z = self.rt_z / self.maxReward
        # self.rtHat_z = self.rtHat_z / self.maxReward

        # -------------------------------------------- #
        self.cT = self._sample(t, self.cT)

        #rt_z: reward given to indivual agent in z  
        return self.rt, self.rtHat, self.rt_z, self.rtHat_z, self.cT, res, resHat, delay

    def _atomicReward(self, nt_z, learnedCost, myLambda):

        rm = np.zeros((self.totalZONES))
        rmHat = np.zeros((self.totalZONES))
        res = np.zeros((self.totalZONES))
        resHat = np.zeros((self.totalZONES))
        delay = np.zeros((self.totalZONES))

        for z in self.planningZONES:
            # tmp[z] = -1 * cost_t_nt_z(z, nt_z[z])
            #penalty of 1 for not reaching 
            #each agent consumes 1 unit of resource
            #individual level
            
            rm_z, rmHat_z, res_z, resHat_z, delay_z = cost_t_nt_z(z, nt_z[z], learnedCost[z], myLambda[z])
            rm[z] = -1 * rm_z
            rmHat[z] = -1 * rmHat_z
            res[z] = -1 * res_z
            resHat[z] = -1 * resHat_z
        ...
        return rm, rmHat, res, resHat, delay
